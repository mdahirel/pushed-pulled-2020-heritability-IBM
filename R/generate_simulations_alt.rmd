...
trt_grid <- tibble(
   VP_logit_dmax = c(0, 1.5^2, 1.5^2, 1.5^2,0,0,0),
   VP_slope = c(0, 6^2, 6^2, 6^2, 6^2, 6^2, 6^2),
   VP_midpoint =c(0, 0.5^2, 0.5^2, 0.5^2, 0.5^2, 0.5^2, 0.5^2), #maybe 0.33 rather than 0.5? adjust medians below if so
   heritability = c(0, 0, 0.33, 1, 0, 0.33, 1)
...
tab <- exp_h2 %>%
  ...
  mutate(replicateID = paste("dmax", dmax_start, "slope", slope_start, "midpoint", midpoint_start, "VP_dmax", (VP_logit_dmax > 0), "VP_DDD", (VP_slope > 0),"mortality", (dispersal_mortality >0), "h2", heritability, reproduction, seedID, sep = "_")) %>%
