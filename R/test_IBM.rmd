...
trt_grid <- expand_grid(
  ...
  reproduction_nl=c("\"clonal\"")  ### version of the variable for entry in netlogo via nlrx; needs escape characters for the quote chars within quote chars
  ) %>%
  ...
  mutate(
    reproduction = str_remove_all(reproduction_nl,'\\"'), #it's a mess with the escape characters to escape the escape characters, but it works
...
nl_object@experiment <- experiment(
  ...
  metrics.turtles = list(
    "turtles"=c("ind_fecundity",
                "genotype_logit_disp0","noise_logit_disp0",
                "genotype_disp_slope","noise_disp_slope",
                "x_birth","disp",
                "momID","PgrandmaID","MgrandmaID",
                "pxcor","who") #we save the pxcor of the turtle, rather than the xcor, because it makes patch-turtle joining easier downscript
  ),
...
tab <- exp_h2 %>%
  dplyr::select(ticks, 
                ...
                heritability,
                fecundity, K, reproduction, seedID = `random-seed`, 
  ...
  mutate(metrics.patches = map(metrics.patches,
    .f = ~ .x %>%
...
filter(pxcor<=1 | pxcor>= (present_front -1)) #we keep info from (the two) core patch(es) and (the two) edge patch(es)
```
