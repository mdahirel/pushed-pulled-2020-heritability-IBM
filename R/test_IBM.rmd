...
trt_grid <- expand_grid(
  ...
  reproduction = "sexual"   # IMPORTANT: ONE of "sexual" or "clonal"
) %>%
...
if(unique(repro)[1] == "sexual"){ 
  metrics.turtles <- list(
    "turtles" = c(
      "ind_fecundity",
      "mean (genotype_logit_disp0)", "noise_logit_disp0",
      "mean (genotype_disp_slope)", "noise_disp_slope",
      "x_birth", "disp",
      "momID", "PgrandmaID", "MgrandmaID",
      "pxcor", "who"
    ) # we save the pxcor of the turtle, rather than the xcor, because it makes patch-turtle joining easier downscript
...
Now let's set the experiment design itself:
```{r set-experiment}
...
tab <- tab %>% 
  rename(genotype_disp_slope=`mean (genotype_disp_slope)`,genotype_logit_disp0=`mean (genotype_logit_disp0)`)
...
write_csv(tab, file = here("NetLogo_output","model-output-clonal.csv"))
}

if(unique(trt_grid$reproduction)=="sexual"){
write_csv(tab, file = here("NetLogo_output","model-output-sexual.csv"))
}
