...
trt_grid <- expand_grid(
  disp_slope_mean = c(-1, 0, 1),
  disp0_mean = c(0.1, 0.2),
  heritability = c(0, 0.33, 0.66, 1),
  dispersal_mortality = c(0, 0.1),
  variance_pheno = c(0, 0.5),  # 1 may be too big, in any case, choose by looking at initial distribution
  # of density-disp function; all range should be covered
  ...
  mutate(
    ...
    VP_logit_disp0 = variance_pheno,
    VP_disp_slope = variance_pheno,
...
nl_object@experiment <- experiment(
  ...
  variables = list( ### needs to be a nested list
    ...
    "disp_slope_mean" = list(values = trt_grid$disp_slope_mean),
    "heritability" = list(values = trt_grid$heritability),
    "VP_logit_disp0" = list(values = trt_grid$VP_logit_disp0),
    "VP_disp_slope" = list(values = trt_grid$VP_disp_slope),
...
tab <- exp_h2 %>%
  dplyr::select(ticks,
    disp_slope_mean, disp0_mean,
    VP_logit_disp0, VP_disp_slope,
  ...
  mutate(
    v_f = 2 * sqrt( ## v_f = expected velocity of "equivalent" pulled wave:
      log(fecundity) * (1 - 1 / K) * ## fecundity term at N = 1
        0.5 * invlogit(logit(disp0_mean) + (1 / K) * disp_slope_mean)
    ) ## dispersal term at N = 1
  ) %>%
  unnest(cols = c(metrics.patches)) %>%
  mutate( 
  #### IMPORTANT: replace the -999 by actual NAs
  mean_genotype_logit_disp0 = na_if(mean_genotype_logit_disp0, -999),
  mean_genotype_disp_slope = na_if(mean_genotype_disp_slope, -999),
  mean_noise_logit_disp0 = na_if(mean_noise_logit_disp0, -999),
  mean_noise_disp_slope = na_if(mean_noise_disp_slope, -999),
  var_genotype_logit_disp0 = na_if(var_genotype_logit_disp0, -999),
  var_genotype_disp_slope = na_if(var_genotype_disp_slope, -999),
  var_noise_logit_disp0 = na_if(var_noise_logit_disp0, -999),
  var_noise_disp_slope = na_if(var_noise_disp_slope, -999),
  ) %>% 
  filter(N_predispersal > 0 | N_postdispersal > 0) %>% ##we only keep populated patches
  mutate(replicateID = paste("slope", disp_slope_mean, "d0", disp0_mean, "VP", VP_disp_slope, "h2", heritability, reproduction, seedID, sep = "_")) %>%
