...
trt_grid <- tibble(
   ...
   VP_slope = c(0, 0.012^2, 0.012^2, 0.012^2),
   VP_midpoint =c(0, 0.25^2, 0.25^2, 0.25^2), #maybe 0.33? adjust medians below if so
   heritability = c(0, 0, 0.33, 1)
  # (because if V_P=0, then any h^2 will give the same V_A than h^2=0)
  ) %>% 
  expand_grid(
  dmax_median = 0.5, #median + VP here give relatively uniform distri over ]0-1[
  slope_median = c(-0.012, 0, 0.012), #0, and + or -1SD from 0
  midpoint_median = c(0.25, 0.5, 0.75),#same logic here
...
nl_object@experiment <- experiment(
  ...
  runtime = duration,
  evalticks = c(10, 50),#100),#150), ## do we only pick those from the start, or save every 10s and filter dataset after (would help see if speed and evo have converged)
