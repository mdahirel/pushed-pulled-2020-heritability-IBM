...
library(arm)         # CRAN v1.12-2
library(matrixStats) # CRAN v0.61.0

library(mgcv)        # CRAN v1.8-38
...
- their life stage (juvenile, adult, coded as a binary variable);
...
- their carrying capacity $K$ ($\geq 0$ integer; in the present study, $K$ is fixed and constant across all patches);
...
**Figure S2.2**. Distribution of the increases in density needed to go from 5% of $d_{max}$ to 95%, assuming $\alpha \sim \mathrm{Normal}(0,500)$. Histogram drawn based on 10000 samples.
...
evo_convergence <- data %>%
  ...
  group_by(
    replicateID, ticks, heritability,
    is.VP_dmax, is.VP_DDD,
    VP_logit_dmax, VP_slope, VP_midpoint,
    dispersal_mortality, fecundity
  ) %>%
  summarise(
    var_genotype_logit_dmax = weighted.mean(var_genotype_logit_dmax, N_predispersal, na.rm = TRUE),
...
tab_all <- tab_all %>%
  mutate(
    fixinterval = ordered(time_to_fix,
...
Here we reproduce the analyses and figures displayed in the main text Results, but for the low-fecundity scenarios. Note that `r sum(is.na(tab_low$advance))+sum(na.omit(tab_low$advance < 0))` out of 2700 replicates were excluded when fitting the velocity models, due to going fully extinct before the window used for velocity models (between $t=100$ and $t=120$) or because they had negative speeds during that window. We chose to exclude a very small percentage and stay consistent to the analyses in the main text, rather than change the analysis method for the Supplementary analyses.
...
tab_low %>%
  filter(is.VP_dmax == 1 & edge_pxcor_120 > 0) %>% # does not make sense to include replicates with no variation in dmax
...
p11 <- tab_low %>%
  filter(is.VP_dmax == 1 & is.VP_DDD == 1 & edge_pxcor_120 > 0) %>%
  ggplot() +
...
p10 <- tab_low %>%
  filter(is.VP_dmax == 1 & is.VP_DDD == 0 & edge_pxcor_120 > 0) %>%
  ggplot() +
  geom_hline(yintercept = 0, lty = 2) +
  geom_vline(xintercept = 0, lty = 2) +
  geom_ribbon(
...
p01 <- tab_low %>%
  filter(is.VP_dmax == 0 & is.VP_DDD == 1 & edge_pxcor_120 > 0) %>%
  ggplot() +
  geom_hline(yintercept = 0, lty = 2) +
  geom_vline(xintercept = 0, lty = 2) +
  geom_ribbon(
