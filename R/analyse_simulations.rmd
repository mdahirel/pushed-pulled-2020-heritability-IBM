...
<!--ADD SOME INTRO FLAVOUR TEXT?-->

After running the code in `R/generate_simulations`, we obtained, and saved, a `csv` file containing patch-level data for every simulation we ran. We can now load it, but we're going to immediately make a quick change: in the IBM, phenotypic variance variables denote, well, the actual phenotypic variances we wanted to input. But since we only tested two levels, variance or no variance, much easier for us to work with binary variables going forward:
...
data <- read_csv(here("NetLogo_output", "simulation_output.csv")) %>%
  mutate(
    ...
    # if VP_slope >0, then VP_midpoint is also >0 in our code
    # so only one binary variable needed for density-dependent dispersal variables
  )
```

OK, the dataset we just imported contains the following variables:

<!--DETAILS TO WRITE OUT LATER-->

# Data wrangling

To do our analyses, we're going to build three "processed" tables from this "raw" dataset, which we'll then merge into a final table.

## sub-table 1: range front data

Here we build a table that contains, for each time x replicate, summary information about the location of the range edge (farthest populated patch), and on mean traits in the range front (defined here as patches < 5 patches from the edge; see Supplementary S2 for justification): 
...
tab_intermediate <- data %>%
  ...
  # we define the range edge as the farthest patch containing individuals
  # after dispersal (so N_postdispersal > 0)
  ungroup() %>% 
  filter(pxcor > edge_pxcor - 5) # and we only keep patches within the front
...
## code slows down *a lot* here for a while, probably due to the size of the summarise call
## tried to fix it a bit, but not worth investing more time
## so expect a wait time of about 5min here before the code carries on:

tab_front <- tab_intermediate %>%
  select(replicateID,ticks,
...
## sub-table 2: starting conditions
...
## sub-table 3: information about genetic diversity loss
...
tab_genet <- tab_intermediate %>%
  ...
  mutate(
    time_to_fix = case_when(
      front_fixed == 1 ~ ticks,
      T ~ 121
    ) ## not fixed yet at end of experiment
  ) %>%
  group_by(
  ...
  ungroup() %>%
  # finally we convert it into an ordered factor:
...
# TO DO: add a read_csv to re-import
# with code to reconvert fixinterval to ordered factor and
# mortality to factor
```
...
#you can check colors using colorblindr::cvd_grid(p1_4)
...
```{r}
## function is copied unmodified from Ben Bolker FAQ 
...
## note: we can ignore the p value, it's for a test of overdispersion
...
The general idea is that underdispersion relative to the binomial may be the sign that the trials that are collapsed together for each response (the generations, here) are not independent. This makes sense here: probability of advancing depend on densities at the front, and we can expect these to be temporally autocorrelated.  Intuitively this should not be as big a problem as overdispersion: underdispersion means the model will exaggerates variance compared to data, which should lead to more conservative results?
...
To find out which of our seven GAMs is the best, let's make a model selection table. We first create function to extract % of deviance explained and R^2^ from the model object, to add them to the selection table:
...
It is extremely clear here what model is the best, independently of the criterion used (density-dependence included, shape-dependent, post-evolution). We keep that in mind for later, and move to the analysis of genetic diversity data.

### genetic diversity: fitting ordinal models

Our measure of the capacity of expansions to keep **neutral** genetic diversity at the front is... the time they take to lose it, i.e. having only one allele at the front. This is time to event, so it should be easy to analyse using survival models. Except we only sampled expansions every 20 generations to save memory. But no worry, this is where discrete time-to-event models come to the rescue. A simple way to fit these is to fit an ordinal model, especially with a cloglog link, since it becomes basically a discrete proportional hazards model. So let's do that, with seven model structures, just like velocity:
...
Here we draw a plot where we show the t=100 value of dispersal capacity/dmax, depending on heritability, mortality and source of variation. We plot the replicate values in grey, and the overall averages in black. We also add the mean starting value (0.5 for everyone) as a dashed line, as a reminder.

Note that for this plot, we obviously excluded replicates where the initial phenotypic variance for dmax was 0.
...
Here we do a broadly similar thing as in Figure 5, but for our density-dependent metric. So there are two big changes actually:
- first, the initial value of the metric varies here depending on replicates, so the base plot is different. We use a (y-x)~x scatterplot where y=trait after evolution (t=100), x=trait at t0. Deviation from the horizontal line denoting no change indicates evolution. In addition, we fit the corresponding linear regression on each subplot

- second, there are three ways phenotype can vary and influence density-dependence (dmax only, matching traits only, both), instead of 2 for dmax (dmax only, dmax and matching traits). So the plot is correspondingly bigger.

Finally, it is possible to identify regions of plotted space that are impossible to evolve into, because a density-dependence that strong would imply dmax higher than the maximal possible dmax for the replicate. So we shaded these in grey.
