...
```{r}
trait_data100<-data %>% 
  filter(ticks==100 & pxcor >= present_front-5 ) %>%  
  group_by(replicateID) %>% 
  summarise(
...
ggplot(tab2)+
  geom_hline(yintercept = 0,lty=2)+
  geom_point(aes(start_uncond0_K,mean_uncond0_K-start_uncond0_K,col=where),alpha=0.1)+
  geom_smooth(aes(start_uncond0_K,mean_uncond0_K-start_uncond0_K,col=where),method="lm")+
  facet_grid(rows=vars(dispersal_mortality,is.VP_dmax),
                       cols=vars(heritability,is.VP_DDD))+ 
...
ggplot(tab2)+
  geom_hline(yintercept = 0,lty=2)+
  geom_point(aes(mean_d0_start,mean_d0-mean_d0_start,col=where),alpha=0.1)+
  geom_smooth(aes(mean_d0_start,mean_d0-mean_d0_start,col=where),method="lm")+
  facet_grid(rows=vars(dispersal_mortality,is.VP_dmax),
                       cols=vars(heritability,is.VP_DDD))+ 
