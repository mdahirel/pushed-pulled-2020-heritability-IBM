...
mod_100=gam(cbind(advance,20-advance)~
              ...
              mortality, 
            family=binomial,data=subset(tab,advance>-1), method = "REML")
...
modsel_table<-model.sel(list(mod_100=mod_100,
               mod_100_K=mod_100_K,
...
modsel_table<-model.sel(list(mod_div_100=mod_div_100,
...
p1<-fitted_samples(mod_100,n=1000,newdata=newdata,seed=42,scale="response") %>% 
  ...
  scale_x_continuous("d0 at t = 100")+
  facet_grid(rows=vars(paste("m = ", mortality)),
                       cols=vars("effect of d0"))+
...
p2<-fitted_samples(mod_100,n=1000,newdata=newdata,seed=42,scale="response") %>% 
  ...
  scale_y_continuous("")+
  scale_x_continuous("slope at t = 100")+
    facet_grid(rows=vars(paste("m = ", mortality)),
                       cols=vars("effect of slope"))+
  coord_cartesian(ylim=c(0,1))
...
p1<-cbind(newdata,predict(mod_div_00,newdata=newdata)$fit) %>% 
  ...
  mutate(name=fct_recode(name,
                                       `< 20`="20",  ###IMPORTANT: add proper <= sign
                                       `21 to 40`="40",
                                       `41 to 60`="60",
                                       `61 to 80`="80",
                                       `81 to 100`="100",
  ...
  scale_y_continuous("Probability of fixation")+
  scale_x_continuous("d0 at t = 0")+
  guides(fill="none")+
  facet_grid(rows=vars(paste("m = ", mortality)),
                       cols=vars("effect of d0"))+
  coord_cartesian(ylim=c(0,1))
...
p2<-cbind(newdata,predict(mod_div_00,newdata=newdata)$fit) %>% 
  ...
  scale_y_continuous("")+
  scale_x_continuous("slope at t = 0")+
