...
```{r}
# we only saved the state at g120, but since patch founding time is saved in each patch, we can reconstruct the dynamics for speed

data |> select(heritability,fecundity,is.VP_dmax,is.VP_DDD,dispersal_mortality,replicateID,founding,pxcor) |> distinct() |> ggplot()+geom_line(aes(founding,pxcor,group=replicateID,col=factor(heritability)))+scale_x_continuous("t (generations since start)")+scale_y_continuous("expansion up to t")+facet_wrap(~fecundity+dispersal_mortality)
```
...
<!--side note: there is an interesting interaction in non-evo treatments between initial beta and initial alpha in shaping speed.

when initial beta is low, increasing initial alpha leads to higher speeds, because we go from a space where dispersal prob decreases over 0-K from a space where it increases (from the same starting point at density 0) so average dispersal increases with alpha

when initial beta is high we have the reverse pattern

this is destroyed by evo

this has nothing to do with any pushed/pulled densitydependence thing, so do not know what to do with it, but will need to account for it if we include speed in the pushed analysis (not planned for now)

if we express alpha and beta instead in terms of d0 and absolute slope, this probs disappears

a model of the type mod=lmer(scale(front)~(scale(start_d0)+scale(start_slopeA_0_K))* Mort * Fec+(1|scenario),data=tab_var_evo)

performs extremely well (r²>>0.95 for all var scenarios)

maybe slide it in supplementary, for the idea that DDD and d0 both shape speed?
-->
...
tab_var_evo = filter(tab,(is.VP_DDD==1 | is.VP_dmax==1) & heritability == 1) #(iii)
...
```{r}
non_evo <- rbind(
  tab_novar |> mutate(type = "no var"),
  tab_var_noevo |>  mutate(type = "non-genetic var")
) |> 
  mutate(l1 = case_when(type=="no var"~lagging, TRUE~NA_integer_),
         l2 = case_when(type=="non-genetic var"~lagging, TRUE~NA_integer_))

bf1 <- bf(l1|trials(front) + subset(type=="no var")~ start_slopeA_0_K * Mort * Fec  + (1|p|scenario),
                      nlf(phi~1/exp(loginvphi)),
                     loginvphi~start_slopeA_0_K * Mort * Fec  + (1|p|scenario),
                  family=beta_binomial(link_phi = "identity"))

bf2 <- bf(l2|trials(front) + subset(type=="non-genetic var")~ start_slopeA_0_K * Mort * Fec  + (1|q|scenario),
                      nlf(phi~1/exp(loginvphi)),
                     loginvphi~start_slopeA_0_K * Mort * Fec  + (1|q|scenario),
                  family=beta_binomial(link_phi = "identity"))


mod_non_evo <- brm(mvbf(bf1+bf2),
                  data=non_evo,backend="cmdstanr",
        seed=42,
                  prior=c(
                    #set_prior("normal(0,1.5)",class="Intercept"),
                    set_prior("normal(0,1)",class="b", resp="l1"),
                    set_prior("normal(0,1)",class="sd", resp="l1"),
                    set_prior("normal(0,1)",class="b",nlpar="loginvphi", resp="l1"),
                    set_prior("normal(0,1)",class="sd",nlpar="loginvphi", resp="l1"),
                    
                    
                    #set_prior("normal(0,1.5)",class="Intercept"),
                    set_prior("normal(0,1)",class="b", resp="l2"),
                    set_prior("normal(0,1)",class="sd", resp="l2"),
                    set_prior("normal(0,1)",class="b",nlpar="loginvphi", resp="l2"),
                    set_prior("normal(0,1)",class="sd",nlpar="loginvphi", resp="l2"),
                    
                    set_prior("lkj(2)",class="cor")
                  )
)

newdata1=select(non_evo,Mort,Fec,type) |> distinct() |> 
  mutate(front=1) |> 
  expand_grid(start_slopeA_0_K=c(-5:5)/10) |> 
  add_epred_draws(mod_non_evo,re_formula=NA,resp="l1") |> 
  mutate(l1=.epred) |> 
  ungroup() |> 
  select(-c(type,.epred))

newdata2=select(non_evo,Mort,Fec,type) |> distinct() |> 
  mutate(front=1) |> 
  expand_grid(start_slopeA_0_K=c(-5:5)/10) |> 
  add_epred_draws(mod_non_evo,re_formula=NA,resp="l2") |> 
  mutate(l2=.epred)|> 
  ungroup() |> 
  select(-c(type,.epred))

left_join(newdata1,newdata2)
```
...
```{r}
mod_gen_novar <- brm(bf(lagging|trials(front)~ start_slopeA_0_K * Mort * Fec  + (1|p|scenario),
                      nlf(phi~1/exp(loginvphi)),
                     loginvphi~start_slopeA_0_K * Mort * Fec  + (1|p|scenario)),
                  family=beta_binomial(link_phi = "identity"),
        ...
        seed=42,
                  prior=c(
                    ...
                    set_prior("normal(0,1)",class="b",nlpar="loginvphi"),
                    set_prior("normal(0,1)",class="sd",nlpar="loginvphi"),
                    set_prior("lkj(2)",class="cor")
                  )
)
...
newdata=select(tab_novar,Mort,Fec) |> distinct() |> 
  mutate(front=1) |> 
  expand_grid(start_slopeA_0_K=c(-5:5)/10) |> 
  add_epred_draws(mod_gen_novar,re_formula=NA,nlpar="loginvphi")

ggplot(newdata)+
  stat_lineribbon(data=newdata,aes(x=start_slopeA_0_K,y=(.epred)),fill="grey",alpha=0.5)+
  facet_grid(rows=vars(paste("fecundity(0)=",Fec)),cols=vars(paste("m=",Mort)))+
  scale_x_continuous("initial slope of DDD (mean dK - D0)")+
  scale_y_continuous("overdispersion 1/phi")+
  theme_bw()+
  theme(legend.position = "none")
...
```{r}
mod_gen_var_noevo=brm(bf(lagging|trials(front)~ start_slopeA_0_K * Mort * Fec  + (1|p|scenario),
                         nlf(phi~1/exp(loginvphi)),
                     loginvphi~start_slopeA_0_K * Mort * Fec  + (1|p|scenario)),
                  family=beta_binomial(link_phi = "identity"),
...
newdata=select(tab_novar,Mort,Fec) |> distinct() |> 
  mutate(front=1) |> 
  expand_grid(start_slopeA_0_K=c(-5:5)/10) |> 
  add_epred_draws(mod_gen_var_noevo,re_formula=NA,nlpar="loginvphi")
...
bf_slope=bf(mean_Kslope~ start_slopeA_0_K * Mort * Fec  + (1|p|scenario),
                      sigma~start_slopeA_0_K * Mort * Fec  + (1|p|scenario)
)

#while slope is bounded and therefore might be beta, we keep it gaussian to keep comparison to the linear expaction new trait = old trait in absence of evol clear
## use bounded gaussian???
...
newdata=select(tab_var_evo,Mort,Fec,is.VP_dmax) |> distinct() |> 
  mutate(front=1) |> 
