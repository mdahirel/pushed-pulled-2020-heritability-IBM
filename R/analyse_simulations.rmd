...
ribboning=tab %>% 
  select(is.VP_dmax,is.VP_DDD,heritability,mortality) %>% 
  distinct() %>% 
  expand_grid(x=c(-100:100)/100)
...
tab %>% 
  ...
  # denotes the strategies possibles (davg>dmax not possible)
    geom_point(aes(start_avgslope,
                                 mean_avgslope_100-start_avgslope),alpha=0.2)+
    geom_smooth(aes(start_avgslope,
                                 mean_avgslope_100-start_avgslope),method="lm")+
