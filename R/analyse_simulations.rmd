...
p11<- tab %>% 
  ...
  ggtitle('C: Individual variation in slope and dmax')
...
p10<- tab %>% 
  ...
  # denotes the strategies possibles (davg>dmax not possible)
    ...
    geom_smooth(aes(start_avgslope,
                    mean_avgslope_100-start_avgslope),method="lm",col="red")+
  ggtitle('B: Individual variation in dmax')
...
p01<- tab %>% 
  ...
  # denotes the strategies possibles (davg>dmax not possible)
    ...
    geom_smooth(aes(start_avgslope,
                    mean_avgslope_100-start_avgslope),method="lm",col="red")+
  ggtitle('A: Individual variation in slope')
...
p11<- tab %>% 
  ...
  ggplot()+
    ...
    geom_ribbon(data=tibble(x=c(-100:100)/100),
                ...
                fill="darkgrey",col="black",alpha=0.5)+
  # denotes the strategies possibles (davg>dmax not possible)
    geom_point(aes(start_davg_0_K,
                   mean_davg_0_K_100-start_davg_0_K),alpha=0.1)+
    geom_smooth(aes(start_davg_0_K,
                    mean_davg_0_K_100-start_davg_0_K),method="lm",col="red")+
  ggtitle('C: Individual variation in slope and dmax')
...
result<- (p01+p10+p11) &
  ...
  scale_x_continuous("") &
  scale_y_continuous("") &
  coord_cartesian(xlim=c(0,0.5),ylim=c(-0.25,0.8))  & 
  facet_grid(rows=vars(paste("m =",dispersal_mortality)),
               cols=vars(paste("h2 =",heritability))) &
  theme_bw()
...
gt <- patchwork::patchworkGrob(result)
gridExtra::grid.arrange(gt, 
                        left = "Change in dAVG at the expanding edge between t = 0 et t = 100", 
                        bottom = "DAVG at t = 0")
```
