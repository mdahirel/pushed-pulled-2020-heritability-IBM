...
test2<-data %>% 
  ...
  mutate(start_uncond0_K=map2(.x=rnorm_slope_start,.y=rnorm_midpoint_start,
                             .f=function(.x,.y){
                               N<-matrix(seq(0,1,0.1),ncol=11,nrow=1000,byrow=TRUE)
                               1/(1+exp(-.x*(N-.y)))}
                             ))  %>% 
  mutate(start_uncond0_K=map2(.x=rnorm_dmax_start,.y=start_uncond0_K,
                              .f=function(.x,.y){
                                a=.x*.y
                                return(mean(rowMeans(a)/rowMaxs(a)))
                              })) %>% 
  unnest(c(start_slopeA_0_K,start_uncond0_K))
