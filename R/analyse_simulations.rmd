...
vstar <- function(fec,disp, mort){
  ...
  if(vstar < 0){vstar <- 0} 
  #if we want to *exclude* these expansions, can convert to NA instead of 0
  # still easy to detect anyway, these will be the only vstar at actual 0, 
  # if rho>1 will bottom at tiny value but still >0
  if(vstar >1){vstar <- 1}
return(vstar)
}
```
...
```{r}
bin_confint <- tibble(x=0:120,n=120) |> 
  group_by(n) |> 
  mutate(lower = map(.x = x,
                       .f = \(x) binom.test(x = x, n = 120)$conf.int[1]),
         upper = map(.x = x,
                     .f = \(x) binom.test(x = x, n = 120)$conf.int[2])) |> 
  unnest(c(lower,upper)) |> 
  ungroup()

bin_polygon <- tibble(x=c(bin_confint$x,rev(bin_confint$x))/120,
                      y = c(bin_confint$upper,rev(bin_confint$lower)))
```

<!--the general no-evo pattern holds if we split by mortality/fec, but we note that at high mortality/low fec there is a dearth of expansions that substantially faster, plots as supplementary, zoom in on on low vstar??-->

```{r}
  ggplot(tab_fix)+
  geom_polygon(data=bin_polygon,aes(x,y),fill="lightgrey")+
  geom_point(aes(vstar,obs_v/120,fill=start_Kslope),pch=21,size=2.5)+
  geom_abline(intercept = 0,slope=1)+
  geom_line(data=bin_confint,aes(x=x/120,y=lower),lty=2)+
  geom_line(data=bin_confint,aes(x=x/120,y=upper),lty=2)+
...
```{r}
## use vstar start or final??? or BOTH and compare
  ggplot(tab_evo)+
  geom_polygon(data=bin_polygon,aes(x,y),fill="lightgrey")+
  geom_point(aes(vstar,obs_v/120,fill=start_Kslope),pch=21,size=2.5)+
  geom_abline(intercept = 0,slope=1)+
