...
Again: we will only focus on the state of our expansions at the end of the simulations (t = 120). There is good evidence that evolutionary dynamics are at or close to equilibrium for most replicates at this time. (see Supplementary)
...
For good measure, we also collect the expansion front location post dispersal:
...
We then get back some information about how many replicates we have in our set vs how many we created (to see how much went extinct during simulations):
...
We remove the replicates that did not went extinct but failed to advance 
<!--OPEN QUESTION: should we remove them only for EVO/GENET AND KEEP THEM FOR SPEED-->
```{r filter_failed_expansions}
