...
tab_edge<-data %>% 
  ...
  ungroup() %>% 
  group_by(replicateID,ticks,dispersal_mortality,heritability,is.VP_dmax,is.VP_DDD,fecundity) %>% 
  summarise(
  ...
  ungroup() %>% 
  pivot_wider(id_cols = c(replicateID,dispersal_mortality,heritability,is.VP_dmax,is.VP_DDD,fecundity),
              names_from=ticks,
              values_from=c(mean_dmax, mean_davg_0_K, mean_d0, 
                            mean_avgslope,mean_Kslope,mean_uncond,
                            front,N_sedentary,N_predispersal))
```
...
tab_genet <- data %>% 
  ...
  mutate(time_to_fix=case_when(edge_fix==1 ~ ticks,
                               T~121)##not fixed yet at end of experiment
         ) %>% 
  group_by(replicateID,dispersal_mortality,heritability,is.VP_dmax,is.VP_DDD,fecundity) %>% 
...
tab<-tab %>% 
  filter(fecundity==5)
```
(for the main text, we focus on the high fecundity case)
...
opti=optimize(f=v,d=d0[i],fec=f0[i],lower=0,upper=50)
...
tab2 <-tab %>% 
  mutate(d0_eff=(1-dispersal_mortality)*mean_d0_100,
         r0_eff=log(fecundity-(dispersal_mortality*mean_d0_100)*fecundity),
         vf=2*(r0_eff*0.5*d0_eff)^0.5) %>% 
...
tab2 %>% 
  ...
  geom_point(aes(vf, ##use effective d0, accounting for motality
                 advance/20,col=mortality,shape=factor(fecundity)))+
...
tab2 %>% 
    filter(mean_avgslope_100>-0.005 & mean_avgslope_100<0.005) %>% 
  lm(advance/20~vf,data=.,subset=vf<1) %>% 
  summary()



tab3=tab2 %>% 
  mutate(vfx=vf*(vf<1)+1*(vf>1)) %>% 
  mutate(expected=20*vfx)

hist(tab3$ratio)

mod=mgcv::gam(cbind(advance,20-advance)~s(expected)+s(mean_avgslope_100,by=mortality)+mortality,data=subset(tab3,fecundity==5),family=binomial)
