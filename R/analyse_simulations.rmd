...
p_d0_m0.1<- draw(mod_100,
                 select="s(mean_d0_100):mortality0.1",rug=FALSE)+
  scale_x_continuous("")+
  scale_y_continuous("")+
  coord_cartesian(ylim=c(-5,25))+facet_grid(cols=vars("effect of d0"))

p_d0_m0.5<- draw(mod_100,
                 select="s(mean_d0_100):mortality0.5",rug=FALSE)+
  scale_x_continuous("")+
  scale_y_continuous("Effect on velocity (logit scale)")+
  coord_cartesian(ylim=c(-5,25))

p_d0_m0.9<- draw(mod_100,
                 select="s(mean_d0_100):mortality0.9",rug=FALSE)+
  scale_x_continuous("low-density dispersal rate d0")+
  scale_y_continuous("")+
  coord_cartesian(ylim=c(-5,25))

p_slope_m0.1<- draw(mod_100,
                    select="s(mean_avgslope_100):mortality0.1",rug=FALSE)+
  scale_x_continuous("")+
  scale_y_continuous("")+
  coord_cartesian(ylim=c(-5,25))+facet_grid(cols=vars("effect of slope"),rows = vars("m=0.1"))

p_slope_m0.5<- draw(mod_100,
                    select="s(mean_avgslope_100):mortality0.5",rug=FALSE)+
  scale_x_continuous("")+
  scale_y_continuous("")+
  coord_cartesian(ylim=c(-5,25))+facet_grid(rows = vars("m=0.5"))

p_slope_m0.9<- draw(mod_100,
                    select="s(mean_avgslope_100):mortality0.9",rug=FALSE)+
  scale_x_continuous("slope")+
  scale_y_continuous("")+
  coord_cartesian(ylim=c(-5,25))+facet_grid(rows = vars("m=0.9"))

p_mortality<-draw(mod_100,rug=FALSE)[[7]]+
  scale_x_discrete("Dispersal mortality m")+
  scale_y_continuous("")

((p_d0_m0.1[[1]]+p_slope_m0.1[[1]])/
    (p_d0_m0.5[[1]]+p_slope_m0.5[[1]])/
    (p_d0_m0.9[[1]]+p_slope_m0.9[[1]])/
    p_mortality)&
  theme_bw() &
  theme(plot.title=element_blank(),
        plot.subtitle = element_blank())&
  geom_hline(yintercept=0,lty=2)##brackets needed because draw outputs patchworks, even when only one plot is output
## so we get the internal ggplot before repatchworking it the way we want
##rug off in draft to save time, on on final??
```
