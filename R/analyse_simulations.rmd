...
library(matrixStats) # CRAN v0.59.0
...
After running the code in `R/generate_simulations`, we obtained, and saved, a `csv` file containing patch-level data for every simulation we ran. We can now load it:

```{r import-data}
raw_data <- read_csv(here("NetLogo_output", "simulation_output.csv"))
```

We're going to immediately make a quick change: in the IBM, phenotypic variance variables denote, well, the actual phenotypic variance we wanted to input. But since we only tested two levels, variance or no variance, much easier for us to work with binary variables going forward:
...
tab_intermediate <- data %>%
  ...
  mutate(edge_pxcor = max(pxcor * (N_postdispersal > 0))) %>%
...
## from here we summarise every key phenotypic trait with weigthed averages
...
Now we can mix and combine these datasets for analyses (and save the merged table, since it's going to be useful for supplementaries):
...
# but asusming no density dependence (avgslope = 0)
newdata <- tibble(
  mean_avgslope_100 = 0,
