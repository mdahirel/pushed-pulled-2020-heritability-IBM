...
data %>% 
  ...
  group_by(replicateID) %>% 
  mutate(vldmax_final=subset(var_genotype_slope,ticks==120) %>% mean()) %>% 
  mutate(relative_vldmax=var_genotype_slope-vldmax_final) %>%
  ungroup() %>% 
  ggplot()+
  geom_line(aes(ticks,relative_vldmax,group=replicateID),col="lightgrey")+
  stat_lineribbon(aes(ticks,relative_vldmax),.width=c(0.001,0.95),point_interval = median_qi,
                  alpha=0.5)+
  facet_grid(rows=vars(dispersal_mortality,is.VP_dmax),
               cols=vars(heritability,is.VP_DDD))+
  geom_hline(yintercept = c(-0.025,0,0.025),lty=2)+
...
mod=glm(cbind(front,ticks-front)~(scale(mean_d0)+scale(mean_slopeA_0_K))*mortality,family=binomial,data=test)
## very basic: ignore possible effects of sources of variances

mod_div=glm(cbind(front_lag,front-front_lag)~(scale(mean_d0)+scale(mean_slopeA_0_K))*mortality,family=binomial,data=test)
...
#gam version?
mod=gam(cbind(front,ticks-front)~s(mean_d0, by = mortality,k=10)+s(mean_slopeA_0_avg,by=mortality,k=10)+mortality,
        family=binomial,data=test,method="REML")

mod_div=gam(cbind(front_lag,front_100-front_lag)~s(mean_d0, by = mortality)+s(mean_slopeA_0_K,by=mortality)+mortality,
        family=binomial,data=test,method="REML")
...
mod=brm(front|trials(ticks)~ (scale(mean_d0)+scale(mean_slopeA_0_K))*mortality,
...
mod_div=brm(front_lag|trials(front)~ (scale(mean_d0_100)+scale(mean_slopeA_0_K))*mortality,
        data=test,family=binomial,
        chains=4,iter=2000,warmup=1000,
        prior=c(set_prior("normal(0,1)",class="Intercept"),
                set_prior("normal(0,1)",class="b")),
...
test2<-data %>% 
  ...
  filter(pxcor == present_front - 5 ) %>%  # higher N so less noise than front - 1, so estimates of mean more accurate
  filter(ticks==120) %>% 
  select(is.VP_DDD,is.VP_dmax,mean_slopeA_0_avg,
         mean_slopeA_0_K, mean_uncond0_K,
         VP_slope,VP_midpoint,VP_logit_dmax,mean_dmax,mean_slope,mean_midpoint,mean_d0,mean_dK,
         dispersal_mortality,N_predispersal,
         heritability,
         replicateID,seedID,
