...
Here we build a table that contains, for each time x replicate combination, summary information about the location of the range edge (farthest populated patch after dispersal): 
...
#### a small detour to check data

```{r}
sum(tab$edge_pxcor_120==0)
```

Two runs (out of 2700!) have not advanced by the end of the experiment, so we will exclude them from the analyses of genetic and phenotypic diversity at the front (since with only one patch populated, there is no front by any definition).

Let's use the opportunity to check at the same time the expansions with fecundity = $\log(1.5)$, in prevision of the supplementary:

```{r}
sum(na.omit(tab_all$edge_pxcor_120)==0)
## so across all the replicates, only 23 replicates survived and did not advance
## which means 23-2=21 in teh low fecundity set
sum(is.na(tab$edge_pxcor_120)) # this variable is NA if the expansion is fully extinct before the end
## none in the high fecundity set
sum(is.na(tab_all$edge_pxcor_120)) # 43 in the full set, so all in the low fecundity set
```
...
```{r fig5}
tab %>%
  filter(is.VP_dmax == 1 & edge_pxcor_120 > 0) %>% 
  # does not make sense to include replicates with no variation in dmax
  # we also exclude the replicates that did not advance (because if no front, no evolution at the front)
